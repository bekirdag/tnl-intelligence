...
Date: 2026-07-31
Status: Published and OIDC release-ready — Activepieces runtime canary pending
...
| Future CI/CD releases        | Complete | npm trust `244c5abf-cb1d-4e5b-b089-9abbce207af7` binds `.github/workflows/release-activepieces.yml`, repository `bekirdag/tnl-intelligence`, environment `npm`, and `publish` permission |
...
| Trusted publisher                              | Verified GitHub binding ID `244c5abf-cb1d-4e5b-b089-9abbce207af7`, workflow `release-activepieces.yml`, repository `bekirdag/tnl-intelligence`, environment `npm`, permission `publish` |
| GitHub workflow publication                    | Local and remote main are commit `95194b2`; GitHub API returns `.github/workflows/release-activepieces.yml` from main                                                                   |
...
GitHub main and local HEAD both resolve to commit `95194b2`; GitHub serves
`.github/workflows/release-activepieces.yml` from main. npm independently lists
the matching trusted-publisher binding with `publish` permission. Future
versions must be published only through this workflow.

The remaining validation is product-level rather than registry-level: install
the public package in an Activepieces instance and run the six-action runtime
canary against a live TNL account.
